...
int fat_begin_write(struct super_block *sb)
{
	struct msdos_sb_info *sbi = MSDOS_SB(sb);

	down_read(&sbi->write_gate);
	if (READ_ONCE(sbi->write_blocked)) {
		up_read(&sbi->write_gate);
		return -EROFS;
	}
	return 0;
}
EXPORT_SYMBOL_GPL(fat_begin_write);

void fat_end_write(struct super_block *sb)
{
	up_read(&MSDOS_SB(sb)->write_gate);
}
EXPORT_SYMBOL_GPL(fat_end_write);

long fat_disable_write(struct file *file)
{
	struct super_block *sb = file_inode(file)->i_sb;
	long ret;

	down_write(&MSDOS_SB(sb)->write_gate);
	WRITE_ONCE(MSDOS_SB(sb)->write_blocked, true);
	...
	if (ret)
		WRITE_ONCE(MSDOS_SB(sb)->write_blocked, false);
	up_write(&MSDOS_SB(sb)->write_gate);
	return ret;
}
EXPORT_SYMBOL_GPL(fat_disable_write);
		   ...
		   void (*setup)(struct super_block *))
{
	...
	init_rwsem(&sbi->write_gate);
	sbi->write_blocked = false;

/*
 * hash.c - SHA-256/CRC32C hash library.
 *
 * hash_sha256_fd uses a producer/consumer ring: the producer reads with pread
 * and the consumer computes SHA-256. The footer payload hash is SHA-256;
 * CRC32C is used only for the descriptor checksum.
 */
...
struct ring {
    struct ring_slot slots[RING_SLOTS];
...
static void *producer_thread(void *arg)
{
    ...
    while (offset < args->total_size) {
        ...
        do {
            head = atomic_load_explicit(&args->ring->head,
        ...
        bytes_read = pread(args->fd, args->ring->slots[slot].data,
                           aligned_want, (off_t)offset);
...
static void *consumer_thread(void *arg)
{
    ...
    for (;;) {
        ...
        do {
            tail = atomic_load_explicit(&args->ring->tail,
                                        memory_order_acquire);
            if (head < tail)
                break;
            if (atomic_load(&args->ring->done) && head >= tail)
                goto finish;
            sched_yield();
        } while (1);
        ...
        head++;
        atomic_store_explicit(&args->ring->head, head,
                              memory_order_release);
    }
...
int hash_sha256_fd(int fd, uint64_t len, uint8_t digest[32], int chunk_kib)
{
    ...
    if ((size_t)chunk_kib > SIZE_MAX / 1024U)
        return -1;
    ...
    fprintf(stderr,
            "  hash_sha256_fd: %llu bytes, chunk=%d KiB, ring=%d slots, "
